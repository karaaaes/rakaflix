...
<div class="flex flex-col gap-[14px] max-w-max">
    <form action="{{ route('member.transaction') }}" method="post">
        @csrf
        <input type="text" name="package_id" id="package_id" value="{{ $user_premium->package_id }}" hidden>
    ...
    <form action="{{ route('member.subscription.destroy', $user_premium->id) }}" method="post">
        @csrf
        @method('delete')
        <button type="submit" class="py-[13px] px-[58px] outline outline-1 outline-stream-gray outline-offset-[-3px] rounded-full text-center">
